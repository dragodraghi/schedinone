...
export default function LoginPage({ onLogin, error, accessCode = "" }: Props) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden" style={{ background: "var(--bg-deep)" }}>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-8 text-center relative z-10 animate-in">
        ...
        <div className="flex gap-3">
          <button type="button" onClick={handleFillCode} disabled={!accessCode}
            className="btn-glow flex-1 py-3 rounded-xl font-bold tracking-wider transition-all glass text-center disabled:opacity-40"
            style={{
              fontFamily: "Outfit, sans-serif",
            ...
            {codeJustInserted ? (
              <div>
                ...
                <div className="text-[8px] mt-0.5 normal-case tracking-normal" style={{ color: "var(--text-muted)" }}>
                  {accessCode} · copiato anche negli appunti
                </div>
              </div>
            ) : (
              <div>
