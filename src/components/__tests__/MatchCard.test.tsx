...
describe("MatchCard", () => {
  it("renders team names", () => {
    render(<MatchCard match={match} prediction={null} onPredict={vi.fn()} />);
    expect(screen.getByText("Italia")).toBeInTheDocument();
    expect(screen.getByText("Brasile")).toBeInTheDocument();
  });
  ...
  it("calls onPredict when button is clicked", () => {
    ...
    render(<MatchCard match={match} prediction={null} onPredict={onPredict} />);
    fireEvent.click(screen.getByRole("button", { name: "1" }));
    expect(onPredict).toHaveBeenCalledWith("m1", "1");
  });
  it("highlights selected prediction", () => {
    render(<MatchCard match={match} prediction="X" onPredict={vi.fn()} />);
    const xBtn = screen.getByRole("button", { name: "X" });
    expect(xBtn.className).toContain("bg-blue-600");
  });
  ...
  it("shows score when available", () => {
    ...
    render(<MatchCard match={finishedMatch} prediction="2" onPredict={vi.fn()} />);
