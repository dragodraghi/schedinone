    ...
    <div className="match-row glass rounded-lg px-3 py-2 flex items-center gap-2">
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between text-xs">
          <span className="font-medium truncate" style={{ fontFamily: 'Outfit, sans-serif' }}>{match.homeTeam}</span>
          <span className="font-mono mx-1 text-[10px]" style={{ color: 'var(--text-muted)' }}>
            {match.score ?? "vs"}
          </span>
          <span className="font-medium truncate text-right" style={{ fontFamily: 'Outfit, sans-serif' }}>{match.awayTeam}</span>
        </div>
      </div>
      <div className="flex gap-1 shrink-0">
        {signs.map((sign) => {
          ...
          let classes = "sign-btn w-10 h-10 rounded text-xs ";
